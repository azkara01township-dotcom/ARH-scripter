...
local expiredDevicesFile = "/sdcard/Android/media/.com.android.sistem.tmp_87g"

-- ⚙️ KONFIGURASI EXPIRED USER (UBAH BAGIAN INI SAJA)
local expiredUserID = 100                         -- 🔢 Ganti: 20, 30, 40, 50, dst
local expiredCode   = "ARHPremiumTS-2days"        -- 🔑 Kode expired
local expireDate    = "2025-11-10"               -- 📅 Ganti tanggal kadaluarsa
...
-- 📅 Cek apakah expired (pakai waktu Unix agar akurat)
...
-- 📌 Alert login info + auto notifikasi sisa hari
local function showLoginInfo(mode)
    ...
    elseif mode == "Expired Code" then  
        ...
        local leftText
        if daysLeft > 0 then
            leftText = string.format("⏳ %d day%s left", daysLeft, daysLeft ~= 1 and "s" or "")
        elseif daysLeft == 0 then
            leftText = "⚠️ Expiring today!"
        else
        ...
        elseif daysLeft == 0 then
            gg.toast("⚠️ Your code expires today!")
        elseif daysLeft < 0 then
            gg.toast("❌ Your code is already expired.")
        end
    end  
...
if not loginOK and isExpiredDeviceRegistered(expiredCode) then
    if isExpiredDate() then
        ...
        if sisa <= 3 then
            gg.toast("⚠️ Your access will expire in " .. sisa .. " day" .. (sisa ~= 1 and "s" or ""))
        end
    end
end
